...
> 作者：@HiTw93 | 来源：[X (Twitter)](https://x.com/HiTw93/status/2032091246588518683)
...
我觉得最直接的理解方式，是把 Claude Code 拆成六层来看。只强化其中一层，系统就会失衡：CLAUDE.md 写太长，上下文先污染自己了；工具堆太多了，选择就搞不清楚了；subagents 开得到处都是，状态就漂移了；验证这步跳过了，出了问题根本不知道是哪里挂的。
...
**真正要关注的五个层面：** 结果不稳定，查上下文加载顺序，不是模型的事；自动化失控，看控制层有没有设计，不是 agent 太主动；长会话质量下降，中间产物把上下文污染了，换个新会话比反复调 prompt 有用得多。
...
对于复杂重构、迁移、跨模块改动，这样做比"急着出代码"有用多了。按两下 Shift+Tab 进入 Plan Mode，**进阶玩法是开一个 Claude 写计划，再开一个 Codex 以"高级工程师"身份审这个计划，让 AI 审 AI，效果很好**。
...
像这种需要在任务中途停下来问用户的场景，他们前后试了三种做法：
...
Hooks 很容易被当成"自动运行的脚本"，但用下来，觉得它更像是把一些不能交给 Claude 临场发挥的事情，重新收回到确定性的流程里。
...
Subagent 就是从主对话派出去的一个独立 Claude 实例，有自己的上下文窗口、只用你指定的工具、干完汇报结果。核心价值不是"并行"，而是**隔离**，扫代码库、跑测试、做审查这类会产生大量输出的事，交给 Subagent 做，主线程只拿摘要，不会被中间过程污染。

Claude Code 内置了 **Explore**（只读扫库，跑 Haiku 省成本）、**Plan**（规划调研）、**General-purpose**（通用），也可以自定义。
...
Compaction（上下文压缩）的执行流程：上下文快满时，Claude Code 开一个 fork 调用，把完整对话历史喂给模型，加一句"Summarize this conversation"，这一步命中缓存所以只需 1/10 的价格，压缩完之后，原来几十轮对话被替换成一段 ~20k tokens 的摘要，System + Tools 还在，再挂上之前用到的文件引用，腾出空间继续新的轮次。
...
春节放假时，用 Claude Code 做了一个开源 terminal 项目 [Kaku](https://github.com/tw93/Kaku)，底层是 Rust + Lua，也带了一些 AI 能力。混合语言加上自定义配置系统，实际折腾下来反而暴露出不少典型的 agent 协作问题。

### 环境透明比你想象中重要

Claude Code 调用的都是真实的 shell、git、package manager 和本地配置。这里面只要有一层不透明，它就只能开始猜；一旦走到猜环境这一步，可靠性通常就会掉得很快。

所以后来很快就在 terminal 里加了个 `doctor` 命令，把环境状态、依赖和配置情况先统一收上来，输出一份结构化的健康报告。Claude Code 开始做事前先跑一次 doctor，确实能省掉很多"环境没搞清楚就开干"的问题。

另外，假如 CLI 本身就有 `init`、`config`、`reset` 这类语义清楚的子命令，Claude Code 用起来会稳不少，比让它自己去猜配置文件怎么摆要靠谱。先把状态收敛住，再暴露编辑入口，顺序一反过来就很容易乱。
...
基于文章里的六层框架，作者把这套检查整理成了一个开源 Skill 项目 [tw93/claude-health](https://github.com/tw93/claude-health)，可以一键检查你的 Claude Code 配置现在处于什么状态。
